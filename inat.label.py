    ...
    for label, iconic_taxon_name in labels:
        ...
        if fungus_fair_mode:
            ...
            if notes_value:
                ...
                notes_safe = re.sub(
                    r"Imported by Mushroom Observer \d{4}-\d{2}-\d{2}", "", notes_safe
                )
...
def main():
    ...
    if args.stack_order:
        ...
        np = args.num_per_page  # number of labels per page
        nr = np // 2  # number of rows per page (assuming 2 columns)
        n = m // np if m % np == 0 else (m // np) + 1  # number of pages
        ...
        for i in range(n * np):
            ...
            # - (i % np) % nr: row within page (0-nr-1)
            # - (i % np) // nr: column within page (0-1)
            # - i // np: which page
            j = n * (2 * ((i % np) % nr) + (i % np) // nr) + i // np
            if j >= m:
        ...
        if labels:
            if args.minilabel:
                if not (rtf_mode or pdf_mode):
                    print_error(
                        "Error: --minilabel requires either --rtf or --pdf output"
                    )
                    sys.exit(1)
                if rtf_mode:
                ...
                for label, _ in labels:
                    for field, value in label:
                        if field == "Notes":
                            ...
                            value = re.sub(
                                r"Imported by Mushroom Observer \d{4}-\d{2}-\d{2}",
                                "",
                                value,
                            )
