...
def register_fonts() -> None:
    ...
    with _fonts_lock:
        ...
        try:
            ...
            pdfmetrics.registerFont(
                TTFont("Liberation Serif-Bold", "LiberationSerif-Bold.ttf")
            )
            pdfmetrics.registerFont(
                TTFont("Liberation Serif-Italic", "LiberationSerif-Italic.ttf")
            )
            pdfmetrics.registerFont(
                TTFont("Liberation Serif-BoldItalic", "LiberationSerif-BoldItalic.ttf")
            )
            pdfmetrics.registerFontFamily(
                ...
                boldItalic="Liberation Serif-BoldItalic",
            )
        except (OSError, ValueError, TTFError) as e:
            print_error(
                f"Warning: Preferred font 'Liberation Serif' not found or invalid: {e}. PDF output may use a fallback."
            )
        ...
        if shutil.which("fc-match"):
            try:
                styles = {
                    "normal": "sans-serif:lang=vi",
                    "bold": "sans-serif:lang=vi:weight=bold",
                    "italic": "sans-serif:lang=vi:slant=italic",
                    "boldItalic": "sans-serif:lang=vi:weight=bold:slant=italic",
                }
                font_paths = {}
                all_found = True
                for style, query in styles.items():
                    ...
                    if path:
                        ...
                        print_error(
                            f"Warning: fc-match found no path for font style '{style}' with query '{query}'."
                        )
                        all_found = False
                        break
                ...
                if all_found:
                    ...
                    pdfmetrics.registerFont(
                        TTFont(f"{family_name}-Bold", font_paths["bold"])
                    )
                    pdfmetrics.registerFont(
                        TTFont(f"{family_name}-Italic", font_paths["italic"])
                    )
                    pdfmetrics.registerFont(
                        TTFont(f"{family_name}-BoldItalic", font_paths["boldItalic"])
                    )
                    pdfmetrics.registerFontFamily(
                        family_name,
                        normal=family_name,
                ...
                print_error(
                    f"Warning: Could not find or register a system Unicode font: {e}. Special characters in PDF may not render correctly."
                )
...
def rl_safe(text: object) -> str:
    ...
    return (
        ...
        .replace("__ITALIC_START__", "<i>")
        .replace("__ITALIC_END__", "</i>")
        .replace("__BOLD_START__", "<b>")
        .replace("__BOLD_END__", "</b>")
    )
...
MO_ORIGINALLY_POSTED_TEXT_RE = re.compile(
    r"Originally posted to Mushroom Observer on [A-Za-z]+\. \d{1,2}, \d{4}\."
)
MO_IMPORTED_TEXT_RE = re.compile(r"Imported by Mushroom Observer \d{4}-\d{2}-\d{2}")
MO_ORIGINALLY_POSTED_RTF_RE = re.compile(
    r"\\line Originally posted to Mushroom Observer on [A-Za-z]+\. \d{1,2}, \d{4}\."
)
MO_IMPORTED_RTF_RE = re.compile(
    r"((\\line)\s+\2+\s+\2 Imported|Imported) by Mushroom Observer \d{4}-\d{2}-\d{2}"
)


def _remove_mo_import_text(text: str) -> str:
    """Remove Mushroom Observer import boilerplate from plaintext-like notes."""
    text = MO_ORIGINALLY_POSTED_TEXT_RE.sub("", text)
    return MO_IMPORTED_TEXT_RE.sub("", text)


def _remove_mo_import_rtf(text: str) -> str:
    """Remove Mushroom Observer import boilerplate from escaped RTF notes."""
    text = MO_ORIGINALLY_POSTED_RTF_RE.sub("", text)
    return MO_IMPORTED_RTF_RE.sub("", text)


def split_hex_string(s: str, n: int) -> str:
    """Split a long hex string into fixed-width lines."""
    return "\n".join([s[i : i + n] for i in range(0, len(s), n)])


def _format_rtf_text(text: str) -> str:
    """Escape text for RTF and convert internal marker tokens to RTF groups."""
    ...
    while attempt < retries:
        ...
        try:
            headers = {
                ...
                "User-Agent": "inat.label.py (label generator)",
            }
            ...
            if response.status_code == 429:
                ...
                if (
                    not notified_patience
                    and (total_wait + wait) >= patience_notice_threshold
                ):
                    print(
                        "Note: experiencing API rate limiting; being patient (up to 30s) to avoid skipping labels.",
                        flush=True,
                    )
            ...
            if 500 <= response.status_code < 600:
                ...
                wait = max(0.5, min(wait, remaining))
                print_error(
                    f"Server error {response.status_code}. Retrying in {wait:.1f}s (attempt {attempt}/{retries})"
                )
                if (
                    not notified_patience
                    and (total_wait + wait) >= patience_notice_threshold
                    ...
                    print(
                        "Note: experiencing server delays; being patient (up to 30s) to avoid skipping labels.",
                        flush=True,
                    )
                time.sleep(wait)
                total_wait += wait
            ...
            wait = max(0.5, min(wait, remaining))
            print_error(
                f"Timeout/SSL error. Retrying in {wait:.1f}s (attempt {attempt}/{retries})"
            )
            if (
                not notified_patience
                and (total_wait + wait) >= patience_notice_threshold
                ...
                print(
                    "Note: experiencing network delays; being patient (up to 30s) to avoid skipping labels.",
                    flush=True,
                )
            time.sleep(wait)
            total_wait += wait
            ...
            wait = max(0.5, min(wait, remaining))
            print_error(
                f"Network error. Retrying in {wait:.1f}s (attempt {attempt}/{retries})"
            )
            if (
                not notified_patience
                and (total_wait + wait) >= patience_notice_threshold
...
def get_taxon_details(taxon_id: int | str) -> ObsData | None:
    ...
    with _taxon_pending_lock:
        ...
        if existing is not None:
            wait_event: threading.Event | None = existing
        else:
...
def get_mushroom_observer_data(
    ...
    if error:
        if "Not found (404)" in error:
            ...
            print(
                f"Error fetching Mushroom Observer observation {mo_id}: {error}",
                flush=True,
            )
        return None, "Life"
    ...
    if data and "results" in data and data["results"]:
        ...
        if isinstance(mo_observation, int):
            print(
                f"Error: Insufficient data from Mushroom Observer API for observation {mo_id}. Skipping.",
                flush=True,
            )
            return None, "Life"
        ...
        if "location" in mo_observation and isinstance(
            mo_observation["location"], dict
        ):
            observation["place_guess"] = mo_observation["location"].get(
                "name", "Not available"
            )
        else:
        ...
        if "location" in mo_observation and isinstance(
            ...
            if (
                "longitude_east" in location
                and "longitude_west" in location
                and "latitude_north" in location
                and "latitude_south" in location
            ):
                longitude = (
                    float(location.get("longitude_east", 0))
                    + float(location.get("longitude_west", 0))
                ) / 2
                latitude = (
                    float(location.get("latitude_north", 0))
                    + float(location.get("latitude_south", 0))
                ) / 2

                observation["geojson"] = {"coordinates": [longitude, latitude]}

        if "consensus" in mo_observation and isinstance(
            mo_observation["consensus"], dict
        ):
            observation["taxon"] = {
        ...
        observation["ofvs"].append({"name": "Mushroom Observer URL", "value": mo_url})
...
def create_inaturalist_label(
    ...
    gps_coords = (
        f"{coords} (±{accuracy})" if accuracy else coords
    )  # accuracy now includes unit (m or km)
    ...
    label = [("Scientific Name", scientific_name)]
    ...
    scientific_name_plain = scientific_name.replace("__ITALIC_START__", "").replace(
        "__ITALIC_END__", ""
    )
    is_redundant = _common_name_is_redundant(common_name, scientific_name_plain)
    ...
    for field_name in dna_fields:
        ...
        if dna_value:
            if isinstance(observation_data["id"], str) and observation_data[
                "id"
            ].startswith("MO"):
    ...
    if debug:
        for f in observation_data.get("ofvs", []):
            print_error(
                "OFV FIELD: " + repr(f.get("name")) + " → " + repr(f.get("value"))
            )
    ...
    if mushroom_observer_url and not (
        isinstance(obs_number, str) and obs_number.startswith("MO")
    ):
        ...
        for rank_marker in [" var. ", " subsp. ", " f. ", " sect. ", " subg. "]:
            if rank_marker in scientific_name:
                scientific_name = scientific_name.replace(
                    rank_marker, f"__ITALIC_END__{rank_marker}__ITALIC_START__"
                )
...
def _select_pdf_font(labels: list[TaggedLabel]) -> tuple[str, float]:
    """Return the PDF base font and font-size multiplier for a label set."""
...
def create_pdf_content(
    ...
    base_font, font_size_multiplier = _select_pdf_font(labels)
        ...
        if fungus_fair_mode:
            ...
            if common_name:
                label_content.append(
                    Paragraph(f"<b>{rl_safe(common_name)}</b>", ff_center_style)
                )
            ...
            if edibility:
                details_text.append(
                    f"<b>Edibility:</b> {rl_safe(get_pretty_edibility(edibility))}"
                )
            ...
            if edibility:
                ...
                img_name = os.path.join(
                    script_dir, "images", f"{edibility.lower()}.jpg"
                )
            ...
            if img_obj:
                ...
                table_details = (
                    details_p if details_p else Paragraph("", custom_normal_style)
                )
                table = Table(
                    [[table_details, img_obj]], colWidths=[col1_width, col2_width]
                )
                table.setStyle(
                    TableStyle(
                        [
                            ...
                            (
                                ...
                                (0, 0),
                                0,
                            ),  # No padding on text cell
                            (
                                "RIGHTPADDING",
                                (1, 0),
                                (1, 0),
                                0.5 * inch,
                            ),  # 0.5 inch padding on image cell to move it left
                            (
                                "TOPPADDING",
                                (0, 0),
                                (0, 0),
                                0.25 * inch,
                            ),  # Push text down 0.25 inch
            ...
            if notes_value:
                ...
                notes_safe = _remove_mo_import_text(notes_safe)
                ...
                notes_paragraph = Paragraph(
                    f"<b>Notes:</b> {notes_safe}", custom_normal_style
                )
...
def _minilabel_obs_number(label: LabelFields) -> str | None:
    """Return the observation identifier displayed next to a minilabel QR code."""
    return next(
        (
            v
            for f, v in label
...
def create_minilabel_pdf_content(
    ...
    for label, _ in labels:
        ...
        obs_number = _minilabel_obs_number(label)
    ...
    try:
        for idx, (label, _iconic_taxon_name) in enumerate(labels):
            ...
            if fungus_fair_mode:
                ...
                if spore_print:
                    details_rtf += (
                        r"{\b Spore Print:} " + escape_rtf(spore_print) + r"\line "
                    )
                if edibility:
                    details_rtf += r"{\b Edibility:} " + escape_rtf(
                        get_pretty_edibility(edibility)
                    )
                ...
                for field, value in label:
                    if field == "iNaturalist URL":
                        ...
                        field.startswith("iNat")
                        or field.startswith("iNaturalist")
                        or field.startswith("Mushroom Observer")
                    ):
                        ...
                        if value:
                            ...
                            # MO import cleanup
                            value_rtf = _remove_mo_import_rtf(value_rtf)
                ...
                if not no_qr:
                    qr_hex, qr_size = (
                        generate_qr_code(qr_url, minilabel_mode=False)
                        if qr_url
                        else (None, None)
                    )
                    if qr_hex:
        ...
        return (
            rtf_header + "Error generating content: " + escape_rtf(str(e)) + rtf_footer
        )
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "observation_ids",
        nargs="*",
        help="Observation number(s), URL(s), or CSV file(s) (for fungus fair mode)",
    )
    parser.add_argument(
        "--file",
        metavar="filename",
        help="File containing observation numbers or URLs (separated by spaces, commas, or newlines)",
    )
    output_group = parser.add_mutually_exclusive_group()
    output_group.add_argument(
        "--rtf",
        metavar="filename.rtf",
        help="Output to RTF file (filename must end with .rtf)",
    )
    output_group.add_argument(
        "--pdf",
        metavar="filename.pdf",
        help="Output to PDF file (filename must end with .pdf)",
    )
    parser.add_argument(
        "--find-ca", action="store_true", help="Find observations within California"
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=None,
        help="Max parallel API requests (default 5, or INAT_MAX_WORKERS env)",
    )
    parser.add_argument(
        "--max-wait-seconds",
        type=float,
        default=None,
        help="Max total wait per API call when retrying (default 30s, or INAT_MAX_WAIT_SECONDS env)",
    )
    parser.add_argument(
        "--common-names",
        action="store_true",
        help="Include common names in labels (off by default)",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="Suppress detailed retry messages (e.g., 429 lines); still shows patience notes and summary",
    )
    parser.add_argument("--debug", action="store_true", help="Print debug output")
    parser.add_argument(
        "--no-qr", action="store_true", help="Omit QR code from PDF and RTF labels"
    )
    parser.add_argument(
        "--minilabel",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Field to use as title (only for PDF output)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fungusfair", action="store_true", help="Generate labels for fungus fairs"
    )
    parser.add_argument(
        "--edibility",
        choices=["edible", "nonedible", "poisonous", "unknown"],
        help="Edibility status (fungus fair mode)",
    )
    parser.add_argument(
        "--scientificname", type=str, help="Scientific name (fungus fair mode)"
    )
...
def _parse_custom_fields(args: argparse.Namespace) -> tuple[list[str], list[str]]:
    """Parse --custom additions/removals into separate field-name lists."""
    fields_to_add = []
    fields_to_remove = []
    if args.custom:
        ...
        for arg in custom_items:
            ...
            if arg.startswith("+") or arg.startswith("-"):
                ...
                print_error(
                    f"Error: Invalid format for --custom. Field options must start with '+' or '-'. Found: '{arg}'"
                )
                sys.exit(2)
    return fields_to_add, fields_to_remove


def _merge_bugguide_tokens(observation_ids: list[str]) -> list[str]:
...
def _stack_order(labels: list[TaggedLabel], num_per_page: int) -> list[TaggedLabel]:
    """Reorder labels so cut-and-stack output reads in original order."""
    stacked_labels = []
    m = len(labels)  # number of labels
    labels_per_page = num_per_page
...
def _emit_output(
    ...
    """Write labels to the selected output target."""
    if labels:
        if args.minilabel:
            ...
            render_plaintext_labels(labels)
    else:
...
def _norm_csv_key(s: str | None) -> str:
    """Normalize CSV headers for fungus-fair mode."""
    if not s:
        return ""
...
def _csv_get_val(
    ...
    """Return the first matching non-empty CSV row value."""
    for key in keys:
...
def _split_cli_inputs(
    ...
    if args.fungusfair:
        ...
        if args.file:
            parser.error(
                "--file is not supported with --fungusfair. Pass CSV file(s) as positional args instead."
            )
                    ...
                    for line_num, row in enumerate(reader, start=2):
                        ...
                        if sci_name:
                            manual_label.append(
                                (
                                    "Scientific Name",
                                    f"__ITALIC_START__{sci_name}__ITALIC_END__",
                                )
                            )
                        else:
                            print_error(
                                f"Error on line {line_num}: Missing required value for 'Scientific Name'."
                            )
                            continue
                        ...
                        if normalized_edibility:
                            ...
                            if edibility:
                                print_error(
                                    f"Warning on line {line_num}: Invalid edibility value '{edibility}'. Defaulting to 'unknown'"
                                )
...
def _add_manual_fungusfair_label(
    ...
    if not observation_ids and args.fungusfair and args.scientificname:
        ...
        manual_label.append(
            ("Scientific Name", f"__ITALIC_START__{args.scientificname}__ITALIC_END__")
        )
        if args.commonname:
...
def _print_generation_estimate(total_requested: int) -> None:
    if total_requested > 25:
        ...
        if seconds > 0 or (
            hours == 0 and minutes == 0 and total_requested > 0
        ):  # Include seconds if it's less than a minute, but only if there are labels to generate
        ...
        if not time_parts:  # Handle case where estimated_time is 0
            ...
            if len(time_parts) > 1:
                ...
                time_str_human_readable = (
                    time_str_human_readable.rsplit(", ", 1)[0]
                    + " and "
                    + time_str_human_readable.rsplit(", ", 1)[1]
                )

        print(
            f"Generating {total_requested} labels, this will take about {time_str_human_readable}",
            flush=True,
        )
